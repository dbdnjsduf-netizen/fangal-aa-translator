...
const changelogData: Version[] = [
  {
    ...
    date: "2026.08.01",
    changes: [
      "일반 수동 선택을 브라우저 문자 범위 방식에서 세로수동과 같은 주황색 박스 드래그 방식으로 전환",
      "박스 안에 중심점이 들어온 실제 문자만 범위로 분리해 인접한 AA·공백·다른 대사의 위치를 보존",
      "수동·세로수동 상태에서 기존 자동 감지 항목을 짧게 클릭해 선택을 끄거나 다시 켜는 공통 토글 추가",
      "세로쓰기 한 글자를 클릭해도 같은 말풍선 그룹 전체가 함께 전환되고 번역 완료 항목은 변경하지 않도록 보호"
    ]
  },
  {
    version: "3.2.0",
