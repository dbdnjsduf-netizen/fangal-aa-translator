...
const changelogData = [
  {
    ...
    changes: [
      '세로쓰기·수동정규식·금지항목 처리까지 백그라운드에서 실행해 대용량 문서의 화면 멈춤 완화',
      '스마트 분석의 현재 단계와 진행률·처리 행 수를 표시하고 모드 전환 시 작업을 즉시 종료',
      '밀집 AA의 세로쓰기 후보 조합 폭증을 억제해 세로 영역 분석 정체 방지',
      '수동정규식 추가와 연속 적용을 비동기로 처리하고 문자열 검색·위치 조회를 최적화',
      '화면 밖 행의 렌더링을 미루고 드래그 범위의 행만 측정해 긴 문서의 표시·선택 속도 개선',
    ],
  },
  {
    version: '1.11.11',
    date: '2026-08-11',
    title: '게시물 문맥 번역과 자동선택 정밀화',
    changes: [
      '약 50개 목표점 근처의 2ch 게시물 헤더를 우선 청크 경계로 사용하고 게시물별 문맥을 분리',
      '같은 게시물의 앞뒤 대사로 생략된 주어·지시어·말투를 판단하고 이어지는 관형절을 종결형으로 끊지 않음',
      '독립 숫자·한자·문장부호, 영문 약어·한자, 하트 대사와 점선 상태창의 자동선택 보강',
      '여러 행의 밀집 AA 안에 있는 라틴·작은 가나·반복 한자 눈썹 질감을 그림으로 제외',
      '여러 조각으로 나뉜 긴 수동정규식을 전체 문자열 한 덩어리로 우선 복원',
    ],
  },
  {
    version: '1.11.10',
    date: '2026-08-10',
    title: '번역 프롬프트 전면 개편과 내용 확인',
    changes: [
      'AI에 실제 전달되는 전체 프롬프트를 확인·복사하고 사용자 스타일은 별도 탭에서 편집',
      '구형 세로쓰기 글자 수·띄어쓰기 지시를 제거하고 내부 배치 표시는 모델에 전달하지 않음',
      '의미·말투·항목 순서 보존과 실패 재번역 지시를 현행 번역 처리 방식에 맞춰 전면 재작성',
      '화자 추측 대신 현재 원문의 실제 어미로 AA 고유 말투를 적용하고 다른 대사로의 전파를 차단',
      '이전 기본 프롬프트는 자동 갱신하고 사용자가 작성한 커스텀 스타일은 그대로 보존',
    ],
  },
  {
    version: '1.11.9',
    date: '2026-08-10',
    title: 'AA 캐릭터 고유 말투 보존',
    changes: [
      '야루오·야라나이오·야라나이코의 원문 어미를 각 한국어 캐릭터 말투로 대응',
      '데키루오·스이세이세키·카나리아의 경어와 고유 종결어를 자연스럽게 보존',
      '뎃테이우·츠루야상·츄루야상의 밈 말투와 웃음 표현을 한국어로 재현',
      '원문 표식과 확인된 화자에만 적용해 다른 대사로 캐릭터 말투가 번지는 현상을 방지',
    ],
  },
  {
    version: '1.11.8',
    date: '2026-08-10',
    title: '짧은 말풍선 대사와 캐릭터 말투 보강',
    changes: [
      '닫힌 말풍선 안의 짧은 한자·가나·장음·조사 조합을 선택하고 상자 밖 AA 조각은 계속 제외',
      '장음표가 섞인 가나 효과음과 전각 공백으로 이어진 우측 독립 문장을 한 덩어리로 선택',
      '검증된 상자 안의 단일 한자는 수동정규식으로 선택하되 닫히지 않은 가짜 상자는 차단',
      '원문에 야루오 특유의 종결어가 실제로 있을 때만 ~다오 계열 말투를 보존',
    ],
  },
  {
    version: '1.11.7',
    date: '2026-08-09',
    title: '독립 대사·상태창 자동선택 보강',
    changes: [
      '실제 수동정규식 사례를 바탕으로 가나 효과음·짧은 한자 반응·숫자 조합을 안전한 공간에서 범용 선택',
      '풀이 괄호·말더듬·히라가나와 가타카나 이름 혼합 대사를 큰 공백으로 분리된 경우 자동 선택',
      '말줄임표 뒤 한 글자 반응은 사방 여백이 확인될 때 문장부호까지 선택',
      '긴 이중 경계로 닫힌 상태창의 일본어 라벨과 값은 선택하고 테두리·숫자 전용 영역은 보존',
      '눈썹·반복 문자 질감 AA 억제와 비례 글꼴 폭 보정을 유지하고 한 글자 규칙의 오른쪽 줄 끝을 허용',
    ],
  },
  {
    version: '1.11.6',
    date: '2026-08-09',
    title: '문맥 기반 AA 감지와 실행 안정화',
    changes: [
      '렉과 AA 오선택을 늘리던 실험적 학습형 수동정규식을 제거하고 정확 문자열 규칙만 유지',
      '말풍선 닫힘·실제 글꼴 폭·상하좌우 여백·주변 AA 연결 밀도를 직접 검사하는 문맥 감지로 교체',
      '짧은 실제 대사는 보호하고 눈썹·눈꺼풀·반복 문자 질감 AA는 더 엄격하게 제외',
      '한 글자 수동정규식은 좌우 2칸과 위아래 2줄이 모두 비어 있을 때만 적용',
      'Saitamaar 표시 폭 보정, 숨김 서버 실행·마지막 탭 종료 연동, 이미지 내보내기 지연 로딩',
    ],
  },
  {
    version: '1.11.5',
    date: '2026-08-08',
    title: '수동정규식 선택 우선순위 보정',
    changes: [
      '자동 감지 후 전체 선택에서 제외된 항목도 수동정규식으로 명시하면 자동 선택',
      '완성된 자동 대사는 내부의 한 글자 수동정규식 때문에 다시 분리되지 않도록 보호',
      '혼합 폭 공백·장식용 점·띄어진 느낌표가 섞인 대사를 문장 전체로 선택',
      '금지항목이 수동정규식보다 우선하는 기존 안전 규칙 유지',
    ],
  },
  {
    version: '1.11.4',
