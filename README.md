...
6. 반복되는 장식이나 번역하면 안 되는 오인식은 먼저 **전체 선택**한 뒤, 하단의 **금지하기**를 켜고 해당 선택 항목을 클릭합니다. 완전히 같은 일반 원문 또는 세로쓰기 문장들이 함께 해제되고 다음 파일에서도 제외됩니다.
7. 실수로 금지한 항목은 상단의 **금지항목 관리**에서 삭제합니다. 목록은 브라우저에 보관되며 JSON으로 백업·복원할 수 있습니다.
8. **일괄 번역**을 실행하고 결과를 확인한 뒤 다운로드합니다.
...
현재 공개 버전은 **1.6.0**입니다. 아래 기록은 일반 사용자가 체감하는 주요 기능을 기준으로 묶었습니다.
